...
const FeaturedSection = () => {
    ...
    // redirect this page to all cars page by this function
  ...
  return (
    <motion.div initial={{opacity: 0, y:40}} whileInView={{opacity:1, y:0}} transition={{duration:1, ease:"easeOut"}} className="flex flex-col items-center py-24 px-6 md:px-16 lg:px-24 xl:px-32">
        ...
        <motion.div initial={{opacity: 0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:1, delay:0.5}} >
            <Title
            title="Featured Cars"
            subTitle="Discover our premium cars tailored for your next unforgettable journey"
            />
        </motion.div>


        <motion.div initial={{opacity: 0, y:100}} whileInView={{opacity:1, y:0}} transition={{duration:1, delay:0.5}} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-18">
        ...
        {/* button to open all cars */}
        <motion.button initial={{opacity: 0, y:20}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.6, duration:0.4}} onClick={()=>{
        navigate("/cars");
        scrollTo(0,0);
      }}
      className='flex items-center justify-center gap-2 px-6 py-2 border border-borderColor hover:bg-gray-50
      rounded-b-md mt-18 cursor-pointer'>
